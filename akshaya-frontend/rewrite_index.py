...
old_backend_data_type = '''type BackendData = {
  message_id: number;
...
new_backend_data_type = '''type AnswerData = {
  summary: string;
  eligibility: string[];
  documents: string[];
  next_steps: string[];
...
content = content.replace(old_backend_data_type, new_backend_data_type)

# 3. Update MessageBubble Image rendering
old_image_render = '''{message.imageUri ? (
            <ExpoImage
...
new_image_render = '''{message.imageUri ? (
...
old_assistant_text = '''<FormattedText 
              text={message.text} 
              style={[
                styles.messageText,
...
new_assistant_text = '''<Text style={[styles.messageText, { color: colors.text, fontFamily: Platform.OS === 'ios' ? 'System' : 'sans-serif' }]}>
              {cleanText(message.backendData?.answer?.summary || message.text)}
            </Text>'''
content = content.replace(old_assistant_text, new_assistant_text)

# 5. Rewrite AdvisoryDetails
advisory_start = content.find('function AdvisoryDetails({ data }: { data: BackendData }) {')
advisory_end = content.find('const styles = StyleSheet.create({')

new_advisory_details = '''function AdvisoryDetails({ data }: { data: BackendData }) {
...
if advisory_start != -1 and advisory_end != -1:
    content = content[:advisory_start] + new_advisory_details + "\n\n" + content[advisory_end:]
